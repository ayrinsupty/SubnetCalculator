...
function change_subnet_number()
{
    ...
    else
    {
        if((c % 1) !== 0)
        {
          alert("Enter an integer - no decimal or commas, please!")
        }
        else
        {
          if((c > 999) || (c < 2))
          ...
          {
            paragraph = "<span class ='column'> Subnet Names: </span> <span class ='tab'></span> <span class = 'column'> Number of hosts: </span><br>";
            for(var b = 1; b <= c; b++)
            {
...
var sl = "/";
var pre = document.getElementById("input_prefix").value;
pre = sl + pre;
h = h + pre.replace(/^\s+|\s+$/g,"");
if(!validate(h))
{
  document.getElementById("not_valid_ip").innerHTML = "<b>Invalid network!!</b>";
  return;
}
...
var s = "<p>The network " + h + " has " + g + " hosts.<br>Your subnets need " + a + " hosts.</p>";
var t = "<table id='printTable' cellpadding='3' border='1'><tr><td>Name</td><td>Hosts Needed</td><td>Hosts Available</td><td>Unused Hosts</td><td>Network Address</td><td>Slash</td><td>Mask</td><td>Usable Range</td><td>Broadcast</td><td>Wildcard</td></tr>";
var b = c;
var k = 0;
